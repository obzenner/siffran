...
test("subagent is allowed, budget Block denies, and transport failure closes", async () => { let mode="allow"; const pi=setup(async (r:any) => { if (r.command.type === "StartRun") return allow; if (mode === "fail") throw new Error("offline"); return mode === "deny" ? block : allow; }); await pi.command("empirica").handler("goal", fakeCtx()); const gate=pi.toolCall(); assert.equal((await gate({toolName:"subagent",toolCallId:"1",input:{agent:"worker"}},fakeCtx())), undefined); mode="deny"; assert.equal((await gate({toolName:"subagent",toolCallId:"2",input:{agent:"worker"}},fakeCtx()))!.block,true); mode="fail"; assert.equal((await gate({toolName:"subagent",toolCallId:"3",input:{agent:"worker"}},fakeCtx()))!.block,true); });
